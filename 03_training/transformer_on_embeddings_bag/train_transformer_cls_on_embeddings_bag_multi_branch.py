...
parser.add_argument("--preproc_config_level_1_branch_1", type=str, required=True)
parser.add_argument("--preproc_config_level_1_branch_2", type=str, required=True)

parser.add_argument("--train_config_level_1_branch_1", type=str, required=True)
parser.add_argument("--train_config_level_1_branch_2", type=str, required=True)
...
preproc_config_level_1_branch_1 = args.preproc_config_level_1_branch_1
preproc_config_level_1_branch_2 = args.preproc_config_level_1_branch_2

train_config_level_1_branch_1 = args.train_config_level_1_branch_1
train_config_level_1_branch_2 = args.train_config_level_1_branch_2
...
def create_balanced_bag_subset(labels, minority_class_ratio=0.2, rnd_seed=38):
    ...
    if sampling == 'sqrt': 
        ...
        class_weights = (sqrt_class_occurence / sqrt_class_occurence.sum()).astype(np.float32) ## HERE SQRT SAMPLING
...
def train_loop(cur, y_train_all, y_val_all, results_dir, 
    ...
    for epoch in range(0, num_epochs):
        ...
        _, balance_idx_1, = create_balanced_bag_subset(y_train_all,
        ...
        train_dataset_1 = h5_Dataset(conf_preproc_branch_1["emb_h5"], train_df.iloc[balance_idx_1])  
        
        # create the pytorch data loader
        train_dataset_loader_1 = torch.utils.data.DataLoader(train_dataset_1, 
                                                           batch_size=batch_size,
                                                           num_workers=6, shuffle=False)
        
        # FOR BRANCH 2
        # generate indices to balance out training set
        _, balance_idx_2, = create_balanced_bag_subset(y_train_all,
                                                     minority_class_ratio=conf_train.minority_class_ratio,
                                                     rnd_seed=int(epoch*1.5+3*epoch))
        
        # here actually getting the subset needed
        train_dataset_2 = h5_Dataset(conf_preproc_branch_2["emb_h5"], train_df.iloc[balance_idx_2])  
        
        # create the pytorch data loader
        train_dataset_loader_2 = torch.utils.data.DataLoader(train_dataset_2, 
                                                           batch_size=batch_size,
                                                           num_workers=6, shuffle=False)
        ...
        # Iterate over data
        for (data1, label1), (data2, label2) in zip(train_dataset_loader_1, train_dataset_loader_2):

            data1 = data1.to(device, dtype=torch.float, non_blocking=True)
            label1 = label1.to(device, dtype=torch.long, non_blocking=True)
            
            data2 = data2.to(device, dtype=torch.float, non_blocking=True)
            label2 = label2.to(device, dtype=torch.long, non_blocking=True)
            
            logits, Y_prob, Y_hat, _, _ = model(data1, data2)

            loss = loss_fn(logits, label1)
        ...
        train_loss /= len(train_dataset_loader_1)
        train_error /= len(train_dataset_loader_1)
                

        ## VALIDATION
        # FOR BRANCH 1
        val_dataset_1 = h5_Dataset(conf_preproc_branch_1["emb_h5"], val_df)

        # create the pytorch data loader
        val_dataset_loader_1 = torch.utils.data.DataLoader(val_dataset_1, 
                                                         batch_size=128, 
                                                         num_workers=7, shuffle=False)
        
        # FOR BRANCH 2
        val_dataset_2 = h5_Dataset(conf_preproc_branch_2["emb_h5"], val_df)

        # create the pytorch data loader
        val_dataset_loader_2 = torch.utils.data.DataLoader(val_dataset_2, 
                                                         batch_size=128, 
                                                         num_workers=7, shuffle=False)

        stop, val_loss_epoch, auc_epoch, prauc_epoch = validate( model, val_dataset_loader_1, val_dataset_loader_2,
...
def validate(model, loader_1, loader_2, n_classes, loss_fn = None, results_dir=None):
    ...
    with torch.no_grad():
        for (batch1, batch2) in zip(loader_1, loader_2):
    ...
    val_error /= len(loader_1)
    val_loss /= len(loader_1)
...
for n_experiment in range(conf_train.n_experiment):
    ...
    for i in range(FOLD, FOLD+1):
        ...
        print('Done!')
        train_df = pd.read_csv(f'{conf_preproc_branch_1[splits]}train_split_stratified_{i}.csv')
        val_df = pd.read_csv(f'{conf_preproc_branch_1[splits]}val_split_stratified_{i}.csv')
